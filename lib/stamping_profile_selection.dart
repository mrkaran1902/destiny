...
import 'stamping_mode.dart';
import 'package:flutter/material.dart';

class StampingProfileSelection extends StatefulWidget {
  const StampingProfileSelection({super.key});

  @override
  State<StampingProfileSelection> createState() => _TreatmentPageState();
}

class _TreatmentPageState extends State<StampingProfileSelection> {
  ...
  final Map<String, List<String>> _options = {
    'AREA': ['ForeHead', 'Cheeks', 'Upper Lips', 'Chin', 'Neck'],
            ...
            onPressed: _canProceed()
                ? () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => StampingModePage(
...
Widget _areabuildSelector(String category, List<String> options, Color color) {
  return Column(
    children: options.map((option) {
      ...
      return Padding(
        padding: const EdgeInsets.symmetric(vertical: 34.0, horizontal: 55.0),
        child: SizedBox(
